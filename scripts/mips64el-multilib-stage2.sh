...
export SRC=${SCRIPT}/../src/mips64el-multilib64-linux/stage2
export BUILD=${SCRIPT}/../build/mips64el-multilib64-linux/stage2
...
[ -f "config.log" ] || HOST_CC=gcc \
  ...
  --build=${CROSS_HOST} --host=${CROSS_TARGET} --libdir=/tools/lib64 \
...
cd gcc-build
[ -f "config.log" ] || CC="${CC} ${BUILD64}" \
  CXX="${CXX} ${BUILD64}" ${SRC}/gcc-${GCC_VERSION}/configure \
  --prefix=/tools \
  --libdir=/tools/lib64 --build=${CROSS_HOST} --host=${CROSS_TARGET} \
  --target=${CROSS_TARGET} --with-local-prefix=/tools --enable-long-long \
