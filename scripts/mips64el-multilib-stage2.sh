...
[ -d "${SRCMUL64ELSTAGE2}" ] || mkdir -p "${SRCMUL64ELSTAGE2}"
[ -d "${BUILDMUL64ELSTAGE2}" ] || mkdir -p "${BUILDMUL64ELSTAGE2}"
...
[ -f "config.log" ] || HOST_CC=gcc \
  ...
  --build=${CROSS_HOST} --host=${CROSS_TARGET64} --libdir=/tools/lib64 \
...
[ -f "config.log" ] || CC="${CC} ${BUILD64}" \
  ./configure --prefix=/tools  --libdir=/tools/lib64 || die "***config zlib error"
...
pushd ${BUILDMUL64ELSTAGE2}
[ -d "binutils-build" ] || mkdir binutils-build
cd binutils-build
[ -f "config.log" ] || CC="${CC} ${BUILD64}" \
  ${SRCMUL64ELSTAGE2}/binutils-${BINUTILS_VERSION}/configure \
  ...
  --with-ppl=/tools --with-cloog-prefix=/tools --enable-cloog-backend=isl \
...
cd gcc-build
[ -f "config.log" ] || CC="${CC} ${BUILD64}" \
  CXX="${CXX} ${BUILD64}" ${SRCMUL64ELSTAGE2}/gcc-${GCC_VERSION}/configure \
  --prefix=/tools \
  --libdir=/tools/lib64 --build=${CROSS_HOST} --host=${CROSS_TARGET64} \
  --target=${CROSS_TARGET64} --with-local-prefix=/tools --enable-long-long \
  ...
  || die "***config gcc error"
  cp -v Makefile{,.orig}
  sed "/^HOST_\(GMP\|PPL\|CLOOG\)\(LIBS\|INC\)/s:-[IL]/\(lib\|include\)::" \
      Makefile.orig > Makefile  || die "***sed gcc error"
...
ln -sv bash /tools/bin/sh || die "***link bash error"
popd
...
[ -f "config.log" ] || CC="${CC} ${BUILD64}" \
  ./configure --prefix=/tools --libdir=/tools/lib64 --build=${CROSS_HOST} \
