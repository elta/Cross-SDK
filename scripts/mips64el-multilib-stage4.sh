...
# BEGIN EGLIBC ++
sudo touch ${PREFIXMUL64EL}/etc/ld.so.conf

# Configure EGLIBC +++
sudo cat > ${PREFIXMUL64EL}/etc/nsswitch.conf << "EOF"
...
sudo cat > ${PREFIXMUL64EL}/etc/ld.so.conf << "EOF"
...
# BEGIN RSYSLOG +++
sudo cat > ${PREFIXMUL64EL}/etc/rsyslog.conf << "EOF"
