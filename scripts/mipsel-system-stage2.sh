...
[ -d "${SRCMIPSELSTAGE2}" ] || mkdir -p "${SRCMIPSELSTAGE2}"
[ -d "${BUILDMIPSELSTAGE2}" ] || mkdir -p "${BUILDMIPSELSTAGE2}"
...
pushd ${BUILDMIPSELSTAGE2}
[ -d "binutils-build" ] || mkdir binutils-build
cd binutils-build
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ${SRCMIPSELSTAGE2}/binutils-${BINUTILS_VERSION}/configure \
  --prefix=/tools --build=${CROSS_HOST} --host=${CROSS_TARGET32} \
  --target=${CROSS_TARGET32} --with-lib-path=/tools/lib --disable-nls \
...
cd gcc-build
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  CXX="${CXX} ${BUILD32}" ${SRCMIPSELSTAGE2}/gcc-${GCC_VERSION}/configure \
  --prefix=/tools --disable-multilib \
  --build=${CROSS_HOST} --host=${CROSS_TARGET32} \
  --target=${CROSS_TARGET32} --libexecdir=/tools/lib \
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  CXX="${CXX} ${BUILD32}" ./configure --prefix=/tools \
  --with-shared --build=${CROSS_HOST} --host=${CROSS_TARGET32} \
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ...
  --host=${CROSS_TARGET32} || die "***config bison error"
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ./configure --prefix=/tools --build=${CROSS_HOST} \
  --host=${CROSS_TARGET32} --enable-install-program=hostname \
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ./configure --prefix=/tools --build=${CROSS_HOST} \
  --host=${CROSS_TARGET32} || die "***config diffutils error"
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ./configure --prefix=/tools --build=${CROSS_HOST} \
  --host=${CROSS_TARGET32} --cache-file=config.cache \
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ./configure --prefix=/tools --build=${CROSS_HOST} \
  --host=${CROSS_TARGET32} || die "***config file error"
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ./configure --prefix=/tools --build=${CROSS_HOST} \
  --host=${CROSS_TARGET32} || die "***config gawk error"
...
[ -f "config.log" ] || CC="${CC} ${BUILD32}" \
  ./configure --prefix=/tools --build=${CROSS_HOST} \
  --host=${CROSS_TARGET32} --disable-perl-regexp \
